...
__Ultroid {ultroid_version}, Created  by @Dextrov__
"""
    ...
    if event.is_group and event.sender_id in sed:
        ...
        if not udB.get("STARTMSG"):
            if udB.get("PMBOT") == "True":
                ok = "Puedes contactar a mi maestro usando este bot!!\n\nEnvía tu mensaje, se lo entregaré al maestro."
            await event.reply(
                f"Hola [{get_display_name(u)}](tg://user?id={u.id}), este es el asistente de @Dextrov de [{ultroid_bot.me.first_name}](tg://user?id={ultroid_bot.uid})!\n\n{ok}",
