...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("Importado con éxito " + shortname)
    else:
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext = "\nDisclaimer:\nEste archivo se pega solo aquí SOLO aquí,"
            ftext += "\nwe registrado solo hecho de error y fecha,"
            ftext += "\nrespetamos tu privacidad,"
            ftext += "\n no puede informar este error si ha"
            ftext += "\ncualquier dato confidencial aquí, nadie verá sus datos\n\n"
            ftext += "--------COMIENCE EL REGISTRO DE RASTREO DEL BOT DEL USUARIO--------"
            ftext += "\nFecha: " + date
            ftext += "\nGrupo ID: " + str(errors.chat_id)
            ftext += "\nRemitente ID: " + str(errors.sender_id)
            ftext += "\n\nActivador de evento:\n"
            ...
            ftext += "\n\n--------REGISTRO DE RASTREO DE USERBOT FINAL--------"
            ...
            text = "**Al CAPONE Error report**\n\n"
            link = "[here](https://t.me/Dextrov)"
            text += "Si quieres puedes denunciarlo"
            text += f"- solo reenvía este mensaje {link}.\n"
            text += "No se registra nada excepto el hecho del error y la fecha\n\n"
...
async def progress(
    ...
    """Devolución de llamada de progreso genérico para cargas y descargas."""
    now = time.time()
    diff = now - start
...
def time_formatter(milliseconds: int) -> str:
    ...
    tmp = (
        ...
        + ((str(seconds) + " segundp(s), ") if seconds else "")
        + ((str(milliseconds) + " milisegundos(s), ") if milliseconds else "")
    )
    return tmp[:-2]
