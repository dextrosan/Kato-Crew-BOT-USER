...
• `{i}gban <reply user/ username>`
    Banear globalmente al usuario.

• `{i}ungban <reply user/ username>`
    Desbanear globalmente.

• `{i}listgban`
   Lista de todos los usuarios GBanned.

• `{i}gmute <reply user/ username>`
    Globalmente silencia al usuario.

• `{i}ungmute <reply user/ username>`
    UnMute a nivel mundial.

• `{i}gkick <reply user/ username>`
    Kickear Usuario nivel mundial.

• `{i}gcast <Message>`
    Envíe globalmente ese mensaje en todos los grupos.

• `{i}gucast <Message>`
    Globalmente envía ese mensaje en todos tus usuarios de chat.
"""
...
async def _(e):
    ...
    if e.is_private:
        ...
        return await eod(xx, "`Responder a algún mensaje o agregar su id.`", time=5)
    name = (await e.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(xx, "`El usuario no esta gbanned.`", time=3)
...
async def _(e):
    ...
    if e.is_private:
        ...
        return await eod(xx, "`Responder a algún mensaje o agregar su id.`", tome=5)
    name = (await e.client.get_entity(userid)).first_name
    chats = 0
    if userid == ultroid_bot.uid:
        return await eod(xx, "`No puedo gban yo mismo.`", time=3)
    if str(userid) in DEVLIST:
        return await eod(xx, "`No puedo gban a mis desarrolladores.`", time=3)
    if is_gbanned(userid):
        return await eod(
            xx,
            "`El usuario ya está prohibido y agregado a gbanwatch.`",
            time=4,
        )
    async for ggban in e.client.iter_dialogs():
        if ggban.is_group or ggban.is_channel:
    ...
    gb_msg = f"**#Gbanned** [{name}](tg://user?id={userid}) `en {chats} chats y agregado a gbanwatch!`"
    if reason != "":
        gb_msg += f"\n**Razón** - {reason}"
...
async def gcast(event):
    ...
    if not xx:
        return eor(event, "`Dar algo de texto a Globally Broadcast`")
    tt = event.text
    msg = tt[6:]
    kk = await eor(event, "`Mensaje de difusión global...`")
    er = 0
    done = 0
    async for x in ultroid_bot.iter_dialogs():
        if x.is_group:
            ...
            try:
                ...
                er += 1
    await kk.edit(f"Echo en {done} chats, error en {er} chat(s)")
...
async def gkick(e):
    ...
    chats = 0
    if userid == ultroid_bot.uid:
        return await eod(xx, "`No puedo gkick sacarme.`", time=3)
    if str(userid) in DEVLIST:
