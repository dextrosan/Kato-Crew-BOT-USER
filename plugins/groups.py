...
• `{i}unbanall`
    Desbanear a todos los usuarios

• `{i}rmusers`
    Eliminar usuarios específicamente.
"""
...
async def _(event):
    xx = await eor(event, "Búsqueda de listas de participantes.")
    p = 0
    ...
    async for i in event.client.iter_participants(
        ...
        try:
            ...
            pass
    await eod(xx, "{title}: {p} Desbaneado")
...
async def _(event):
    ...
    if input_str:
        ...
        if not (chat.admin_rights or chat.creator):
            return await eod(xx, "`No eres un administrador aquí!`", time=5)
    p = 0
    b = 0
    c = 0
    d = 0
    ...
    if input_str:
        ...
        required_string += f"  **••Cuentas eliminadas••** `{d}`\n"
