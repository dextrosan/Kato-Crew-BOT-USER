...
• `{i}alive`
    Comprueba si tu bot está funcionando.

• `{i}ping`
    Consulta el tiempo de respuesta del BOT.

• `{i}cmds`
    Ver todos los nombres de PLUGINS.

• `{i}restart`
    Para reiniciar tu bot.

• `{i}logs (sys)`
    Obtenga los registros completos de la terminal.
...
async def lol(ult):
    ...
    header = udB.get("ALIVE_TEXT") if udB.get("ALIVE_TEXT") else "Ey,  Sigo vivo!!."
...
async def heroku_logs(event):
    ...
    await ultroid.send_file(
        ...
        thumb="resources/extras/tenor.gif",
        caption=f"**DEXTROV Registros de Heroku.**\nPegado [Aquí]({url}) también!",
    )
