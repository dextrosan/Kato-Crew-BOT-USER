...
• `{i}unzip <reply to zip file>`
    descomprime el archivo respondido.
"""
...
async def _(ult):
    ...
    msg = await eor(ult, "`Procesando...`")
    if not (
        gt.media
        and gt.media.document
        and gt.media.document.mime_type == "application/zip"
    ):
        return await msg.edit("`Responder a un archivo zip...`")
    k = time.time()
    ...
    dnl = await ultroid_bot.download_media(
        ...
        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
            progress(d, t, msg, k, "Descargando a mi almacenamiento..."),
        ),
    )
    ...
    Enum = 0
    Elist = "**Se produjeron errores al descomprimir**\n\n"
    ...
    if Enum > 0:
        if len(Elist) < 4096:
            ...
            await ultroid_bot.send_message(
                Var.LOG_CHANNEL,
                "UnzipError.txt",
                caption=f"`Se produjo un error al descomprimir el comando",
            )
