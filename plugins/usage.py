...
• `{i}usage`
    Obtenga un uso general.

• `{i}usage heroku`
   Obtén estadísticas de heroku.

• `{i}usage redis`
   Obtenga el uso de redis.
"""
...
async def usage_finder(event):
    ...
    if opt == "redis":
        ...
        await eor(x, "`El qué?`", time=5)


def heroku_usage():
    if HEROKU_API is None and HEROKU_APP_NAME is None:
        return False, "No usas heroku, hermano!"
    useragent = grua()
