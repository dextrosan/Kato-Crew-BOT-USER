...
async def all_messages_catcher(e):
    if udB.get("TAG_LOG"):
        ...
        try:
            ...
            LOGS.info(str(er))
            await ultroid_bot.send_message(NEEDTOLOG, f"Agregue su bot asistente - @{asst.me.username} como administrador aquí.")
    else:
        return
        ...
        if user.id == (await asst.get_me()).id:
            if add == OWNER_ID:
                ...
                return await asst.send_message(
                    Var.LOG_CHANNEL, f"#ADD_LOG\n\nMe habías agregado a {chat}."
                )
            else:
                # , buttons=Button.inline("Leave Chat", data=f"leave_ch_{event.chat_id}|bot"))
                return await asst.send_message(
                    Var.LOG_CHANNEL, f"#ADD_LOG\n\n`{add}` me agregó a {chat}."
                )
...
async def when_ultd_added_to_chat(event):
    if event.user_added:
        ...
        if user.id == OWNER_ID:
            ...
            return await asst.send_message(
                Var.LOG_CHANNEL, f"#ADD_LOG\n\n`{add}` te acabo de agregar a{chat}."
            )
