...
• `{i}destroy <reply to animated sticker>`
    Destruir la pegatina.

• `{i}tiny <reply to media>`
    Para crear  Tiny stickers.

• `{i}convert <gif/img/sticker>`
    Responde al Sticker para convertir en gif o imagen.

• `{i}kang <reply to image/sticker>`
    Kang the sticker (add to your pack).

• `{i}round <reply to any media>`
    Para extraer ronda sticker.

• `{i}waifu <text>`
    pegar texto al azar stickers.
...
def deEmojify(inputString: str) -> str:
    """Eliminar emojis y otros caracteres no seguros de la cadena"""
...
async def waifu(animu):
    ...
    if not text:
        if animu.is_reply:
            ...
            await xx.edit("`No has escrito ningún artículo, Waifu se va.`")
            return
    ...
    try:
        ...
        await xx.edit("`Jefe ! No puedo usar cosas en línea aquí...`")
    except ChatSendStickersForbiddenError:
        await xx.edit("Lo siento jefe, no puedo enviar el Sticker aquí !!")
